...
const archetypeOverrides = Array.from({ length: 10 }, (_, i) => `archetype-profiles-pilot-${String(i + 1).padStart(2, '0')}.json`)
  .flatMap(f => readJson(f).profiles).filter(p => !['alchemist','antipaladin','arcanist','barbarian','bard','bloodrager','brawler','cavalier','cleric','druid','fighter','gunslinger','hunter','inquisitor','investigator','kineticist','magus','medium','mesmerist','monk','monk-unchained','ninja','occultist','oracle','paladin','psychic','ranger','rogue','samurai','shaman','shifter','skald'].includes(p.parentClassId))
  .concat(...['slayer','summoner-unchained','alchemist','antipaladin','arcanist','barbarian','bard','bloodrager','brawler','cavalier','cleric','druid','fighter','gunslinger','hunter','inquisitor','investigator','kineticist','magus','medium','mesmerist','monk','monk-unchained','ninja','occultist','oracle','paladin','psychic','ranger','rogue','samurai','shaman','shifter','skald'].map(id => readJson(`archetype-profiles-${id}.json`).profiles));
...
test('resolveAllProfiles produces 44 class-paths + 1093 archetypes = 1137 profiles, all with unique ids', () => {
  const all = V2.resolveAllProfiles(classProfiles, archetypeOverrides);
  assert.equal(all.length, 1137);
  assert.equal(new Set(all.map(p => p.id)).size, 1137);
  assert.equal(all.filter(p => p.entityType === 'class-path').length, 44);
  assert.equal(all.filter(p => p.entityType === 'archetype').length, 1093);
});
