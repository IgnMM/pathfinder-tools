...
test('resolveAllProfiles produces 44 class-paths + 424 archetypes = 468 profiles, all with unique ids', () => {
  const all = V2.resolveAllProfiles(classProfiles, archetypeOverrides);
  assert.equal(all.length, 468);
  assert.equal(new Set(all.map(p => p.id)).size, 468);
  assert.equal(all.filter(p => p.entityType === 'class-path').length, 44);
  assert.equal(all.filter(p => p.entityType === 'archetype').length, 424);
});
