...
const archetypes = Array.from({length: 10}, (_, index) => String(index + 1).padStart(2, '0')).flatMap(batch =>
...
).filter(profile => profile.parentClassId !== 'alchemist').concat(
  ...
  readJson('assets/find-your-class/v2/archetype-profiles-alchemist.json').profiles
);
...
test('all 44 class profiles and 194 archetype profiles are present and marked valued', () => {
  ...
  assert.equal(archetypes.length, 194);
  ...
  assert.equal(catalogue.counts.valuedArchetypes, 194);
});
