...
const archetypes = Array.from({length: 10}, (_, index) => String(index + 1).padStart(2, '0')).flatMap(batch =>
...
).filter(profile => !['alchemist','antipaladin','arcanist','barbarian','bard'].includes(profile.parentClassId)).concat(
  ...
  ...['alchemist','antipaladin','arcanist','barbarian','bard'].flatMap(id => readJson(`assets/find-your-class/v2/archetype-profiles-${id}.json`).profiles)
);
...
test('all 44 class profiles and 323 archetype profiles are present and marked valued', () => {
  ...
  assert.equal(archetypes.length, 323);
  ...
  assert.equal(catalogue.counts.valuedArchetypes, 323);
});
