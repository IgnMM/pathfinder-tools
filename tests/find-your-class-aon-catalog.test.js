...
test('all 44 class profiles and 1275 archetype profiles are present and marked valued', () => {
  ...
  assert.equal(archetypes.length, 1275);
  ...
  assert.equal(catalogue.counts.valuedArchetypes, 1275);
  assert.equal(catalogue.counts.pendingArchetypes, 0, 'the full catalogue is now valued -- no archetypes remain pending curation');
});
