...
const archetypes = Array.from({length: 10}, (_, index) => String(index + 1).padStart(2, '0')).flatMap(batch =>
...
).filter(profile => !['alchemist','antipaladin','arcanist','barbarian','bard','bloodrager','brawler','cavalier','cleric','druid','fighter','gunslinger','hunter','inquisitor','investigator','kineticist','magus','medium','mesmerist','monk','monk-unchained','ninja','occultist','oracle','paladin'].includes(profile.parentClassId)).concat(
  ...
  ...['alchemist','antipaladin','arcanist','barbarian','bard','bloodrager','brawler','cavalier','cleric','druid','fighter','gunslinger','hunter','inquisitor','investigator','kineticist','magus','medium','mesmerist','monk','monk-unchained','ninja','occultist','oracle','paladin'].flatMap(id => readJson(`assets/find-your-class/v2/archetype-profiles-${id}.json`).profiles)
);
...
test('all 44 class profiles and 898 archetype profiles are present and marked valued', () => {
  ...
  assert.equal(archetypes.length, 898);
  ...
  assert.equal(catalogue.counts.valuedArchetypes, 898);
});
